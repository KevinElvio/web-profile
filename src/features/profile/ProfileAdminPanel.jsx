...
const AboutSection = () => {
  const [formDataAbout, setFormDataAbout] = useState(initialProfile);
  ...
  const loadDataAbout = async () => {
    try {
      ...
      if (savedData) {
        setFormDataAbout(savedData.data.data[0] ?? initialProfile);
      }
    } catch (error) {
      ...
      <div className="space-y-6">
            ...
            <div>
              <input
                ...
                onChange={(e) => {
                  ...
                  if (image) {
                    setFormDataAbout((current) => ({ ...current, image, imagePrev: URL.createObjectURL(image) }));
                  }
                }}
                className="hidden"
                id="profileImage"
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setFormDataAbout((current) => ({ ...current, title: e.target.value }))}
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => setFormDataAbout((current) => ({ ...current, description: e.target.value }))}
            rows={8}
            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
